...
// Tu código acá:
var span =document.querySelector('#createdBy');// <span>Aplicación creada por</span>
span.innerHTML=span.innerHTML + 'Rafael';

  // {
  //   element: 'span',
  //   innerHTML: 'Aplicación creada por: Rafael'
  // }
...
function buildToDo(todo, index) {
  ...
  toDoText.innerHTML=todo.description; //<span> 'Hacer el homework' </span>
  toDoText.setAttribute('id',index); // <span id= '1'> 'Hacer el homework' </span>
  if(todo.complete) {
    toDoText.setAttribute('class','completeText'); //<span id= '1' 
  }                     //class='completeText'> 'Hacer el homework' </span>
  toDoShell.appendChild(toDoText); //<div class='toDoShell'><span id= '1'>'Hacer el homework'</span></div>
  
  toDoText.addEventListener('click',completeToDo); //click sobre un todo
  return toDoShell; // <div class= 'todoShell'><span id= '1'>'Hacer el homework'</span></div>
}
...
function displayToDos() {
  ...
  //div id='toDoContainer'>div, div, div, div </div>
...
function addToDo() {
  // Tu código acá:
  let toDoInput=document.getElementById('toDoInput').value//Hacer el homework
  let newToDo=new ToDo(toDoInput);// {descr: 'Hacer el homework', complete:false}
...
function completeToDo(event) {
  ...
  // Tu código acá:
  toDoItems[index].completeToDo();// {descr:'Correr', complete:true}
  displayToDos();
}
